...
export default function Upgrade({ updateToken }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            response = await fetch(import.meta.env.VITE_BACKEND + "/users/upgrade", option1);
        ...
        try {
            loginResponse = await fetch(import.meta.env.VITE_BACKEND + "/users/login", option2);
    ...
    return (
        <div id="signup-section" className="pb-24 flex flex-col items-center box-border text-black">
            <div id="signup-container" className="mt-20 w-[300px] sm:w-[600px] rounded-md shadow shadow-black dark:shadow-white bg-white">
                <form onSubmit={handleSubmit} id="signup-form" className="w-full h-full p-10 flex flex-col justify-start items-start">
                    <legend className="text-lg w-max font-bold mb-7">Upgrade your blog account</legend>
                    ...
                    <input 
                        ...
                        required
                    />
                    <label htmlFor="userKey">Secret Key</label>
                    <input 
                        type="password" 
                        name="userKey" 
                        onChange={handleInputChange}
        ...
        <p className="text-black dark:text-white mt-2">Already have a blog author account? Log in <Link to="/" className="inline text">here.</Link></p>
    </div>
    )
}